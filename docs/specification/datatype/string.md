...
+ `#!periwinkle Стрічка.підстрічка(початок, кількість)` -  Повертає стрічку, яка складається з символів заданої стрічки, що починаються з переданого індексу та охоплює задану кількість символів. Викидає помилку `ПомилкаІндексу`, якщо індекс більший за довжину масиву.
+ `#!periwinkle Стрічка.розділити(роздільник)` -  Розбиває стрічку на підстрічки за заданим роздільником і повертає масив цих підстрічок.
